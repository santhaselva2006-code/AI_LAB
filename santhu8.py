...
class Factor:
    def __inti__(self,variables,table):
        self.variables = variables
        self.table = table
def multiply(f1,f2):
    vars_new = list(set(f1.variables)|set(f2.variables))
    table_new={}
    for assignment in intertools.product([True,False],repeat=len(vars_new)):
...
def restrict(factor,var,val):
    vars_new=[v for v in factor.variablesv if v != var]
    table_new={}
    for assignment in itertools.product([True,False],repeat=len(vars_new)):
       full = dict(zip(vars_new,assignment))
       full[var]=val
       table_new[assignemnt]=factor.table.get(tuple(full[v]for v in factor.variables),0)
    return Factor(vars_new,table_new)
...
J = Factor(["J","A"],{(True,True):0.9,(False,True):0.1,(True,False):0.05,(False,False):0.95})
M = Factor(["M","A"],{(True,True):0.7,(False,True):0.3,(True,False):0.01,(False,False):0.99})
factors = [B,E,A,J,M]
